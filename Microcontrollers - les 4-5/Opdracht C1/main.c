/*
	...
	* Description: This program sends data to a4-digit display with spi
	* Test configuration:
			MCU:             			ATmega128
			Dev.Board:       			BIGAVR6
...
void writeLEDDisplay(int value){
	...
	if (value < 0)
	{
		...
		writeNum = value / 100;
		value -= writeNum * 100;
		spi_writeWord(3, -1 * writeNum);
		writeNum = value / 10;
		value -= writeNum * 10;
		spi_writeWord(2, -1 * writeNum);
		spi_writeWord(1, -1 * value);
	...
	writeNum = value / 1000;  //4
	value -= writeNum * 1000;
	spi_writeWord(4, writeNum);
	writeNum = value / 100;
	value -= writeNum * 100;
	spi_writeWord(3, writeNum);
	writeNum = value / 10;
	value -= writeNum * 10;
	spi_writeWord(2, writeNum);
